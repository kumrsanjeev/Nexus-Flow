...
st.markdown("""
    ...
    .thinking-box { background-color: #f0f4f9; border-radius: 12px; padding: 15px; color: #0b57d0; border-left: 4px solid #0b57d0; margin: 10px 0; font-size: 0.9rem; }
...
with st.sidebar:
    ...
    if st.button("➕ New Chat"):
        ...
        st.rerun()
    st.markdown("---")
    uploaded = st.file_uploader("Upload PDFs for context", type="pdf", accept_multiple_files=True)
...
if not st.session_state.messages:
    ...
    with col1:
        if st.button("🖼️ Create Image"):
            st.session_state.messages.append({"role":"user", "content":"Ek sundar billi ki realistic photo banao 🐱"})
            st.rerun()
    with col2:
        if st.button("📝 Write Anything"):
            st.session_state.messages.append({"role":"user", "content":"Mujhe coding career ke liye ek motivation post likh kar do 📝"})
            st.rerun()
else:
...
# --- 6. CHAT LOGIC (Strict Language & Image Fix) ---
if prompt := st.chat_input("Ask Gemini"):
    ...
    with st.chat_message("assistant"):
        try:
            ...
            STRICT RULE: Always reply in the SAME LANGUAGE as the user's input. 
            - If user speaks in Hindi, you MUST reply in Hindi. 
            - If Hinglish, use Hinglish.
            - IMAGES: Respond ONLY with [GENERATE_IMAGE: descriptive prompt in English] for images.
            - LOGIC: Use <thinking> tags for reasoning.
            Context: {context}
            """
            ...
            if "[GENERATE_IMAGE:" in final_text:
                ...
                if match:
                    ...
                    # Final URL Fix
                    img_url = f"https://image.pollinations.ai/prompt/{urllib.parse.quote(p_str)}?width=1024&height=1024&nologo=true&seed={np.random.randint(1000)}"
